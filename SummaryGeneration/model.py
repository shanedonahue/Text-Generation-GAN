...
class SummaryModel:
    def __init__(self, **args):
        ...
        if ('load_pretrain' in args) and args['load_pretrain']:
            ...
            self.saver = tf.compat.v1.train.Saver(name=f'Pointer_{date.today().strftime("_%m_%d")}',
    ...
    def _add_placeholder_variable(self):
        ...
        '''
        self.src2idx = tf.compat.v1.placeholder(shape=[self.batch_size, self.max_src_time], dtype=tf.int32,
                                                name='src2idx')
        self.atten2final = tf.compat.v1.placeholder(shape=[self.batch_size, self.max_src_time, 2], dtype=tf.int32,
                                                    name='atten2final')
        self.sum2final = tf.compat.v1.placeholder(shape=[self.batch_size, self.max_sum_time, 3], dtype=tf.int32,
                                                  name='sum2final')
        self.sum2finalidx = tf.compat.v1.placeholder(shape=[self.batch_size, self.max_sum_time], dtype=tf.int32,
                                                     name='sum2finalidx')
        self.src_len = tf.compat.v1.placeholder(shape=[self.batch_size], dtype=tf.int32, name='src_len')
        self.sum_len = tf.compat.v1.placeholder(shape=[self.batch_size], dtype=tf.int32, name='sum_len')
        self.oov = tf.compat.v1.placeholder(shape=[self.batch_size], dtype=tf.string, name='oov')
        ...
        self.coverage_on = tf.compat.v1.placeholder(shape=[], dtype=tf.bool, name='coverage_on')
    ...
    def _construct_encoder(self):
        ...
        with tf.compat.v1.variable_scope('encoder', reuse=tf.compat.v1.AUTO_REUSE):
            self.emb_input = tf.nn.embedding_lookup(params=self.emb, ids=self.src2idx)
            ...
            (self.enc_fw_output, self.enc_bw_output), (
                self.enc_fw_state, self.enc_bw_state) = tf.compat.v1.nn.bidirectional_dynamic_rnn(self.enc_fw_unit,
                                                                                                  self.enc_bw_unit,
                                                                                                  self.emb_input,
                                                                                                  self.src_len,
                                                                                                  dtype=tf.float32)
            ...
            self.enc_dense_c = tf.compat.v1.layers.Dense(self.num_unit, activation=tf.nn.relu,
                                                         kernel_initializer=tf.compat.v1.variance_scaling_initializer(),
                                                         name='encoder_reduce_c_dense')
            self.enc_dense_h = tf.compat.v1.layers.Dense(self.num_unit, activation=tf.nn.relu,
                                                         kernel_initializer=tf.compat.v1.variance_scaling_initializer(),
                                                         name='encoder_reduce_h_dense')

            self.enc_final_state = tf.concat([self.enc_dense_c(self.enc_c_state), self.enc_dense_h(self.enc_h_state)],
                                             axis=1)
    ...
    def _construct_decoder(self):
        ...
        with tf.compat.v1.variable_scope('decoder', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            # For Input
            self.dec_emb_input = tf.TensorArray(tf.float32, size=self.max_sum_time,
                                                element_shape=[self.batch_size, self.emb_dim])
            self.dec_emb_input = self.dec_emb_input.unstack(
                tf.transpose(a=tf.nn.embedding_lookup(params=self.emb, ids=self.sum2finalidx), perm=[1, 0, 2]))
            ...
            self.linear_enc_output = tf.compat.v1.layers.Dense(self.dim_v, use_bias=True, name='linear_enc_output')
            self.linear_dec_state = tf.compat.v1.layers.Dense(self.dim_v, use_bias=False, name='linear_dec_state')
            self.linear_coverage = tf.compat.v1.layers.Dense(self.dim_v, use_bias=False, name='linear_coverage')
            self.v = tf.compat.v1.layers.Dense(1, use_bias=False, name='atten_dense_v')
            ...
            self.linear_vocab_1 = tf.compat.v1.layers.Dense(self.num_unit * 2, activation=tf.nn.leaky_relu,
                                                            name='linear_vocab_1')
            self.linear_vocab_2 = tf.compat.v1.layers.Dense(self.vocab_dim, activation=tf.nn.leaky_relu,
                                                            name='linear_vocab_2')

            # For Final Distribution
            self.linear_context = tf.compat.v1.layers.Dense(1, name='linear_context')
            self.linear_state = tf.compat.v1.layers.Dense(1, name='linear_state')
            self.linear_input = tf.compat.v1.layers.Dense(1, name='linear_input')

            # Recurrent Neural Network (Supervised)
            def next_time_step(step, _state, _input, emb_input, coverage, cov_loss, atten_dist, vocab_dist, final_dist,
                               tokens, p_gen_dist):
                ...
                linear_2 = tf.tile(tf.expand_dims(linear_2, axis=1),
                                   [1, self.max_src_time, 1])  # batch_size * max_src_time * v_dim
                linear_3 = self.linear_coverage(tf.expand_dims(coverage, axis=2))
                e = tf.cond(pred=self.coverage_on,
                            true_fn=lambda: tf.squeeze(self.v(tf.tanh(linear_1 + linear_2 + linear_3))),
                            false_fn=lambda: tf.squeeze(self.v(tf.tanh(linear_1 + linear_2))))
                atten = tf.nn.softmax(e) * self.atten_mask  # batch_size * max_src_time [0, 1]
                normalize = tf.reshape(tf.reduce_sum(input_tensor=atten, axis=1), [-1, 1])
                atten = atten / normalize
                atten_dist = atten_dist.write(step, atten)

                # Calculate Coverage Loss
                cov_loss = cov_loss.write(step, tf.reduce_sum(input_tensor=tf.math.minimum(coverage, atten), axis=1))
                coverage += atten

                # Build Vocabulary Distribution
                context_vec = tf.reduce_sum(input_tensor=self.enc_output * tf.expand_dims(atten, 2),
                                            axis=1)  # batch_size * (num_unit * 2)
                context_state = tf.concat([context_vec, next_state],
                                          axis=1)  # batch_size * (num_unit * 2 + num_unit * 2)
                ...
                ## Calculate p_{gen}
                p_gen = tf.sigmoid(
                    self.linear_context(context_vec) + self.linear_state(next_state) + self.linear_input(_input))
                p_gen_dist = p_gen_dist.write(step, p_gen)

                ## Update attention to final distribution
                vocab_reshape = tf.concat([p_gen * vocab, tf.zeros([self.batch_size, self.final_dim - self.vocab_dim])],
                                          axis=1)
                ...
                # Get Next Token
                next_token = tf.argmax(input=final, axis=1, output_type=tf.int32)
            ...
            last_step, final_state, final_input, emb_input, self.coverage, self.cov_loss, self.atten_dist, self.vocab_dist, self.final_dist, _, self.p_gen = tf.while_loop(
                ...
                loop_vars=(
                    ...
                    tf.TensorArray(tf.float32, size=self.max_sum_time,
                                   element_shape=[self.batch_size, self.max_src_time]),
            ...
            self.atten_dist = tf.transpose(a=self.atten_dist.stack(name='attention_distribution'), perm=[1, 0, 2])
            self.vocab_dist = tf.transpose(a=self.vocab_dist.stack(name='vocabulary_distribution'), perm=[1, 0, 2])
            self.final_dist = tf.transpose(a=self.final_dist.stack(name='final_distribution'), perm=[1, 0, 2])
            self.cov_loss = tf.transpose(a=self.cov_loss.stack(name='covarge_loss'), perm=[1, 0])
            # self.tokens = tf.transpose(self.tokens.stack(name = 'generated_sequence'), perm = [1, 0])
            self.p_gen = tf.transpose(a=tf.squeeze(self.p_gen.stack(name='p_gen')), perm=[1, 0])

            # Unsupervisedly Generate token
            def next_time_step_unsup(step, _state, _input, emb_input, coverage, cov_loss, tokens, p_gen_dist,
                                     gen_prob_dist, is_end):
                next_output, next_state = self.dec_unit(_input, _state)
                ...
                # Get Next Token
                log_prob = tf.math.log(tf.clip_by_value(final, 1e-30, 1.0))
                ...
                next_token = tf.squeeze(tf.random.categorical(logits=log_prob, num_samples=1, dtype=tf.int32))
                next_token = tf.compat.v1.where(tf.equal(is_end, 0), tf.zeros(next_token.get_shape(), dtype=tf.int32),
                                                next_token)

                is_end = tf.compat.v1.where(tf.equal(next_token, 0), tf.zeros(next_token.get_shape()), is_end)

                next_input = tf.nn.embedding_lookup(params=self.emb, ids=next_token)
                next_input = tf.concat([next_input, context_vec], axis=1)
                tokens = tokens.write(step, next_token)

                next_token_mask = tf.one_hot(next_token, self.final_dim, dtype=tf.float32)

                step_log_prob = tf.reduce_sum(input_tensor=tf.multiply(log_prob, next_token_mask), axis=1)
            ...
            self.tokens = tf.transpose(a=self.tokens.stack(name='generated_sequence'), perm=[1, 0])
            self.tokens_len = tf.stop_gradient(tf.clip_by_value(
                self.max_sum_time - tf.reduce_sum(input_tensor=tf.cast(tf.equal(self.tokens, 0), dtype=tf.float32),
                                                  axis=1) + 1, 0., self.max_sum_time))
            self.gen_prob = tf.transpose(a=self.gen_prob.stack(name='generated_sequence_prob'), perm=[1, 0])
    ...
    def _construct_discriminator(self, sum_emb, sum_len):
        ...
        with tf.compat.v1.variable_scope('discriminator', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            with tf.compat.v1.variable_scope('lstm', reuse=tf.compat.v1.AUTO_REUSE):
                ...
                output, state = tf.compat.v1.nn.dynamic_rnn(self.dis_dec_unit, sum_emb, initial_state=state,
                                                            sequence_length=sum_len, swap_memory=False)

                score_layer = tf.compat.v1.layers.Dense(1, name='dis_score_layer')
                reward = tf.squeeze(tf.sigmoid(score_layer(output)), 2)

                reward = tf.pad(tensor=reward, paddings=[[0, 0], [0, self.max_sum_time - tf.shape(input=reward)[1]]],
    ...
    def _construct_baseline(self, sum_emb, sum_len):
        ...
        with tf.compat.v1.variable_scope('baseline', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            with tf.compat.v1.variable_scope('lstm', reuse=tf.compat.v1.AUTO_REUSE):
                ...
                _, state = tf.compat.v1.nn.dynamic_rnn(self.bas_enc_unit, self.emb_input, sequence_length=self.src_len,
                ...
                output, state = tf.compat.v1.nn.dynamic_rnn(self.bas_dec_unit, _sum_emb, initial_state=state,
                                                            sequence_length=sum_len, swap_memory=True)

                score_layer = tf.compat.v1.layers.Dense(1, name='bas_score_layer')
                reward = tf.squeeze(tf.sigmoid(score_layer(output)), 2)
    ...
    def _build_loss_opt(self):
        ...
        # Gather Variables
        var_dec = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='decoder')
        var_enc = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='encoder')
        var_dis = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='discriminator')
        var_bas = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='baseline')
        ...
        self.fake_seq_mask = tf.stop_gradient(
            tf.sequence_mask(tf.cast(self.tokens_len, dtype=tf.int32), self.max_sum_time, dtype=tf.float32))
        self.gather_log_prob = - tf.math.log(tf.clip_by_value(tf.gather_nd(self.final_dist, self.sum2final), 1e-30, 1.))

        self.final_cov_loss = tf.reduce_mean(input_tensor=(tf.reduce_sum(
            input_tensor=tf.multiply(self.seq_mask, self.cov_loss), axis=1) * self.cov_weight) / tf.cast(self.sum_len,
                                                                                                         dtype=tf.float32))
        ...
        self.final_pri_loss = tf.reduce_mean(input_tensor=(tf.reduce_sum(
            input_tensor=tf.multiply(self.seq_mask, self.gather_log_prob), axis=1)) / tf.cast(self.sum_len,
                                                                                              dtype=tf.float32))

        self.target_loss = tf.cond(pred=self.coverage_on, true_fn=lambda: self.final_cov_loss + self.final_pri_loss,
        ...
        # GAN Part
        self.real_loss = tf.math.log(
            tf.reduce_sum(input_tensor=tf.multiply(self.seq_mask, self.real_reward), axis=1) / tf.cast(self.sum_len,
                                                                                                       dtype=tf.float32))
        # self.real_loss = tf.reduce_sum(tf.multiply(self.seq_mask, self.real_reward), axis = 1) / tf.cast(self.sum_len, dtype = tf.float32)
        self.fake_loss = tf.math.log(
            1 - tf.reduce_sum(input_tensor=tf.multiply(self.fake_seq_mask, self.fake_reward), axis=1) / tf.cast(
                self.tokens_len, dtype=tf.float32))
        ...
        self.dis_loss = tf.reduce_mean(input_tensor=- self.real_loss - self.fake_loss)
        ...
        self.gen_loss = - tf.reduce_mean(
            input_tensor=tf.reduce_sum(
                input_tensor=tf.multiply(tf.multiply(self.fake_reward - self.fake_bas, self.gen_prob),
                                         self.fake_seq_mask),
                axis=1
            )
        )
        self.bas_loss = tf.reduce_sum(
            input_tensor=tf.multiply(tf.square(self.fake_reward - self.fake_bas), self.fake_seq_mask))
        ...
        with tf.compat.v1.variable_scope('discriminator', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            # print(self.gen_loss)
            # print(self.dis_loss)
            # print(var_dec)  # filter_variables(var_dec,exclude_patterns=['embedding_matrix'])
            self.dis_opt = tf.compat.v1.train.AdamOptimizer(self.dis_lr, name='disAdam').minimize(self.dis_loss,
                                                                                                  var_list=var_dis,
                                                                                                  global_step=self.dis_global_step_2)
            self.gen_opt = tf.compat.v1.train.AdamOptimizer(self.gen_lr * 0.1, name='genAdam').minimize(self.gen_loss,
                                                                                                        var_list=var_dec,
                                                                                                        global_step=self.gen_global_step_2)
            self.bas_opt = tf.compat.v1.train.AdamOptimizer(self.dis_lr, name='basAdam').minimize(self.bas_loss,
                                                                                                  var_list=var_bas)


    def _build_summary(self):
        self.log_suffix = date.today().strftime('_%m_%d')
        summary_target_loss = tf.compat.v1.summary.scalar('Target_Loss', self.target_loss)
        summary_cov_loss = tf.compat.v1.summary.scalar('Coverage_Loss', self.final_cov_loss)
        summary_prime_loss = tf.compat.v1.summary.scalar('Prime_Loss', self.final_pri_loss)
        summary_dis_loss = tf.compat.v1.summary.scalar('Dis_Loss', self.dis_loss)
        summary_gen_loss = tf.compat.v1.summary.scalar('Gen_Loss', self.gen_loss)
        summary_bas_loss = tf.compat.v1.summary.scalar('Bas_Loss', self.bas_loss)
        self.supervised_summary = tf.compat.v1.summary.merge(
            [summary_target_loss, summary_cov_loss, summary_prime_loss])
        self.unsupervised_summary = tf.compat.v1.summary.merge([summary_dis_loss, summary_gen_loss, summary_bas_loss])
        self.log_writer = tf.compat.v1.summary.FileWriter('./log/', filename_suffix=self.log_suffix)
    ...
    def _beam_search_tf(self):
        ...
        self.atten2final_split = _atten2final_split.unstack(
            tf.expand_dims(tf.slice(self.atten2final, [0, 0, 1], [self.batch_size, self.max_src_time, 1]), axis=1))
        ...
        _enc_output_split = tf.TensorArray(tf.float32, size=self.batch_size,
                                           element_shape=[1, self.max_src_time, self.num_unit * 2])
        self.enc_output_split = _enc_output_split.unstack(tf.expand_dims(self.enc_output, axis=1))

        def next_sample(step, state_array, src_len_array, atten2final_array, enc_output_array, gen_token, gen_prefix,
                        gen_value, gen_atten):
            ...
            initial_token = tf.convert_to_tensor(value=initial_token_np)
            initial_prefix = tf.zeros([self.beam, 1], dtype=tf.int32)
            initial_value = tf.zeros([self.beam, 1], dtype=tf.float32)

            def next_step(inner_step, _input, _state, coverage, _atten_mask, _atten2final, _enc_output, tokens, prefix,
                          value, value_array, atten_array):
                # tile before start
                atten_mask = tf.tile(_atten_mask, [_input.get_shape()[0], 1])

                atten2final_dim0 = tf.tile(
                    tf.reshape(tf.range(0, self.beam, delta=1, dtype=tf.int32), [self.beam, 1, 1]),
                    [1, self.max_src_time, 1])
                atten2final = tf.concat([atten2final_dim0, tf.tile(_atten2final, [_input.get_shape()[0], 1, 1])],
                                        axis=2)
                ...
                linear_2 = tf.tile(tf.expand_dims(linear_2, axis=1),
                                   [1, self.max_src_time, 1])  # beam * max_src_time * v_dim
                linear_3 = self.linear_coverage(tf.expand_dims(coverage, axis=2))
                e = tf.cond(pred=self.coverage_on,
                            true_fn=lambda: tf.squeeze(self.v(tf.tanh(linear_1 + linear_2 + linear_3))),
                            false_fn=lambda: tf.squeeze(self.v(tf.tanh(linear_1 + linear_2))))
                ...
                # Build Vocabulary Distribution
                context_vec = tf.reduce_sum(input_tensor=enc_output * tf.expand_dims(atten, 2),
                                            axis=1)  # beam * (num_unit * 2)
                ...
                ## Update attention to final distribution
                vocab_reshape = tf.concat(
                    [p_gen * vocab, tf.zeros([_input.get_shape()[0], self.final_dim - self.vocab_dim])], axis=1)
                ...
                final = tf.math.log(vocab_reshape + atten_reshape)
                ...
                next_input = tf.concat(
                    [tf.nn.embedding_lookup(params=self.emb, ids=ids), tf.gather(context_vec, beam_ids)], axis=1)
            ...
            cond = lambda step, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11: step < self.max_sum_time
            _, _, _, _, _, _, _, gen_token_array, gen_prefix_array, _, gen_value_array, gen_atten_array = tf.while_loop(
                cond=cond,
                body=next_step,
                shape_invariants=(
                    tf.TensorShape([]),
            ...
            gen_value = gen_value.write(step, tf.squeeze(tf.transpose(a=gen_value_array.stack(), perm=[1, 0, 2])))
            gen_atten = gen_atten.write(step, tf.transpose(a=gen_atten_array.stack(), perm=[1, 0, 2]))
    ...
    def beam_search(self, feed_dict, coverage_on=False, top_k=1):
        ...
        seq, pre, val, att = self.sess.run([self.gen_token, self.gen_prefix, self.gen_value, self.gen_atten],
                                           feed_dict=feed_dict)
        if top_k == 1:
            all_tokens = []
            all_scores = []
    ...
    def train_one_epoch_pre_dis(self, generator, total_n_batch, coverage_on=False,
                                model_name='pointer_unsupervised_dis_pre'):
        '''
            Train for one epoch
    ...
    def train_one_epoch_unsup(self, generator, total_n_batch, coverage_on=False, model_name='pointer_unsupervised'):
        ...
        for feed_dict in tqdm_notebook(generator, total=total_n_batch // (self.n_dis + 1) * (self.n_dis + 1)):
            ...
            if step % (self.n_dis + 1) == 0:
                self.sess.run(
                    [self.bas_opt, self.gen_opt, tf.compat.v1.assign(self.dis_global_step_2, self.gen_global_step_2)],
                    feed_dict=feed_dict)
            else:
